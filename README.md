...
[![Status: open-source prototype](https://img.shields.io/badge/status-open--source_prototype-2563eb)](docs/evidence-boundary.md)
...
Northstar Retail project is a complete, API-key-free vertical slice: review two
real conflicts, approve a strict refund boundary, compile an immutable artifact
...
> artifacts, then shows how a candidate behaves on repeatable sandbox scenarios.
...
performance. All bundled people, orders, and runs are fictional.
...
`$200.01` refund proposal before the sandbox mutates. It preserves the proposal,
policy decision, source rule, and unchanged state as separate evidence.
...
synthetic project. `make demo` runs the API and Next.js app. Docker is optional:
...
source files → immutable documents + exact spans → reviewed Rule IR
    ...
    → fixture adapter → pre-tool policy decision → sandbox tools
...
The default fixture runner is deterministic and never calls a model. It is the
only execution path used by tests and the public demo. Provider protocols and
...
- Browser: five Playwright Chromium paths covering the demo flow, landing
...
For approved, machine-decidable rules, the sandbox tool proxy deterministically
...
- The policy interpreter covers a deliberately small allowlisted AST and only
  the static sandbox tool registry.
- Public-demo reset is appropriate only for fictional data and should be
  protected by `DEMO_RESET_SECRET` if exposed.
