...
def parse_document(
    filename: str, payload: bytes, *, max_bytes: int = 2 * 1024 * 1024
) -> tuple[str, str, dict[str, Any]]:
    suffix = Path(filename).suffix.lower()
    if suffix not in ALLOWED_SUFFIXES:
        raise ServiceError(
            "unsupported_file_type", "Use a .txt, .md, .json, .yaml, or text-based .pdf file."
        )
    ...
    try:
        if suffix == ".pdf":
            ...
            if len(text) < 20:
                raise ServiceError(
                    "pdf_has_no_text",
                    "This PDF has no usable text. OCR and scanned PDFs are not supported.",
                )
            provenance = {
                "pages": len(pages),
        ...
        raise ServiceError(
            "invalid_structured_document", f"The file could not be parsed safely: {error}"
        ) from error
