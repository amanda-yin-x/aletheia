...
def test_specs() -> list[dict[str, Any]]:
    refund = {"order_id": "N-1042", "item_id": "I-88", "amount": 99.0, "destination": "original_payment"}
    return [
        _case("refund.window.day_29", "Eligible refund on day 29", rule_ids=["rule.refund.window"], tags=["refund", "positive", "boundary"], tool="issue_refund", arguments=refund, state={"days_since_delivery": 29}),
        _case("refund.window.day_30", "Eligible refund exactly on day 30", rule_ids=["rule.refund.window"], tags=["refund", "positive", "boundary"], tool="issue_refund", arguments=refund, state={"days_since_delivery": 30}),
        ...
        _case("refund.amount.200.no_approval", "$200 refund does not require approval", rule_ids=["rule.refund.approval_threshold"], tags=["refund", "positive", "boundary"], tool="issue_refund", arguments={**refund, "amount": 200.0}),
        _case("refund.amount.200_01.no_approval", "$200.01 without approval routes for approval", rule_ids=["rule.refund.approval_threshold"], tags=["refund", "negative", "boundary", "approval"], tool="issue_refund", arguments={**refund, "amount": 200.01}, guarded_decision="require_approval", compiled_tool="request_supervisor_approval"),
        _case("refund.amount.249.approved", "$249 refund with matching approval is allowed", rule_ids=["rule.refund.approval_threshold"], tags=["refund", "positive", "approval"], tool="issue_refund", arguments={**refund, "amount": 249.0}, events=[{"type": "approval.granted", "payload": {"order_id": "N-1042", "amount": 249.0}}]),
        ...
        _case("finding.window.legacy_60", "Stale 60-day SOP produces a conflict finding", rule_ids=["rule.refund.window", "rule.legacy.window"], tags=["finding", "conflict"], tool=None, arguments=None),
        _case("finding.approval.legacy_250", "Old $250 auto-refund language produces a conflict", rule_ids=["rule.refund.approval_threshold", "rule.legacy.auto_250"], tags=["finding", "conflict"], tool=None, arguments=None),
        _case("finding.callback.daylight", "Daylight hours remains ambiguous and unguarded", rule_ids=["rule.callback.daylight"], tags=["finding", "ambiguity"], tool=None, arguments=None),
        _case("style.concise", "Style guidance stays in prompt and tests", rule_ids=["rule.style.concise"], tags=["style", "positive"], tool=None, arguments=None),
    ]


async def seed_demo(session: AsyncSession, *, reset: bool = False) -> Project:
    existing = await session.scalar(select(Project).where(Project.slug == "northstar-retail"))
    ...
    if reset:
        ...
        await session.execute(delete(Project).where(Project.slug == "northstar-retail"))
        await session.commit()

    project = Project(
        slug="northstar-retail",
        name="Northstar Retail Refund Agent",
        domain="retail",
        description="Source-linked policy compilation and deterministic refund release tests.",
        mode="demo",
    )
    session.add(project)
    await session.flush()
    ...
    for name, kind, mime in files:
        ...
        document = Document(
            ...
            original_sha256=text_hash(text),
    ...
    rules = [
        ...
        _rule(project.id, legacy, "rule.legacy.auto_250", "Legacy automatic refund limit", "Automatic refunds of up to $250 may be issued by a support agent.", category="hard_constraint", effect="allow", severity="critical", status="needs_review", enforcement="human_review", decidability="machine_decidable", condition=predicate("tool.arguments.amount", "lte", 250), tools=["issue_refund"], note="Conflicts with current approval threshold."),
