...
export default function RunPage() {
  ...
  const ready = guarded?.executed_violation_rate === 0 && guarded?.false_block_rate === 0;
  ...
  return <main className="landing" style={{ maxWidth: 1280, paddingTop: 34 }}>
    ...
    <PageTitle eyebrow="Run comparison · Fixture adapter" title="Release behavior" detail="Prompt and guard effects remain separate across three arms, each starting from an identical case fixture." actions={<Button onClick={() => report.mutate()} disabled={report.isPending}><FileText size={15} /> {report.isPending ? "Creating report…" : "Create evidence report"}</Button>} />
    <div className={`verdict-banner ${ready ? "" : "changes"}`}><CheckCircle2 size={22} /><div><h2>{ready ? "Ready for sandbox pilot" : "Changes required"}</h2><p>{ready ? "No guarded sandbox violations or false blocks were observed in this 16-case fixture suite." : "Review guarded failures before the next sandbox pilot."}</p></div></div>
