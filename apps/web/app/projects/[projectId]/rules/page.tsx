...
export default function RulesPage() {
  ...
  const resolve = useMutation({ mutationFn: (finding: Finding) => api<Finding>(`/api/v1/findings/${finding.id}`, { method: "PATCH", body: JSON.stringify({ resolution_state: "resolved", resolution_note: "Current Refund Policy v3 is authoritative; legacy wording remains in provenance but is excluded from compilation." }) }), onSuccess: refresh });
  ...
  return <div className="content-wrap">
    ...
    <div className="finding-stack">
      {findings.data!.map((finding) => <article key={finding.id} className={`finding-card ${finding.severity} ${finding.resolution_state === "resolved" ? "resolved" : ""}`}>
        <div><strong><Badge tone={tone(finding.severity) as "red" | "amber" | "neutral"}>{finding.proof_status}</Badge> {finding.message}</strong><p>{finding.resolution_state === "resolved" ? finding.resolution_note : `${label(finding.type)} · ${label(finding.severity)} severity`}</p></div>
        {finding.resolution_state === "open" && finding.severity === "critical" ? <Button variant="secondary" className="button-small" disabled={resolve.isPending} onClick={() => resolve.mutate(finding)}><Check size={14} /> Use current policy</Button> : <Badge tone={finding.resolution_state === "resolved" ? "teal" : "neutral"}>{label(finding.resolution_state)}</Badge>}
      </article>)}
    </div>
    ...
    {selected && <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
      <aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="rule-drawer-title">
        ...
        <div className="drawer-body">
          <section className="drawer-section"><h3>Exact source quote</h3><blockquote className="quote">{selected.source_refs[0]?.quote}</blockquote><a className="arrow-label" style={{ marginTop: 9 }} href={`/projects/${projectId}/sources`}>Open {selected.source_refs[0]?.document_name}, lines {selected.source_refs[0]?.line_start}–{selected.source_refs[0]?.line_end} <ExternalLink size={13} /></a></section>
