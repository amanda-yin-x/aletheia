...
export default function OverviewPage() {
  ...
  const nextHref = data.critical_findings ? `/projects/${projectId}/rules` : data.current_build ? `/projects/${projectId}/tests` : `/projects/${projectId}/routing`;
  const nextLabel = data.critical_findings ? "Continue policy review" : data.current_build ? "Run comparison" : "Review placements";
  return <div className="content-wrap">
    ...
    <div className="stat-grid">
      ...
      <StatCard label="Unresolved critical" value={data.critical_findings} note={data.critical_findings ? "Blocks compilation" : "No compilation blockers"} tone={data.critical_findings ? "red" : "teal"} />
    ...
    <section className="panel" style={{ marginBottom: 16 }}>
      ...
      <div className="workflow-track">
        ...
        <div className={`workflow-stage ${!data.critical_findings && !data.current_build ? "current" : data.current_build ? "complete" : ""}`}><span className="stage-icon">3</span>Placements reviewed</div>
        <div className={`workflow-stage ${!data.critical_findings && !data.current_build ? "" : data.current_build ? "complete" : ""}`}><span className="stage-icon">4</span>Bundle compiled</div>
        <div className={`workflow-stage ${data.current_build && !data.last_run ? "current" : data.last_run ? "complete" : ""}`}><span className="stage-icon">5</span>Comparison run</div>
        <div className={`workflow-stage ${data.last_run ? "current" : ""}`}><span className="stage-icon">6</span>Evidence report</div>
      </div>
    </section>
      ...
      <section className="panel">
        ...
        <div className="panel-body">
          <ul className="activity-list">
            ...
            <li><ShieldCheck size={17} /><p><strong>Compiled instruction bundle</strong><small> {data.current_build ? shortHash(data.current_build.content_hash) : "Not compiled"}</small></p><Badge>{data.current_build ? "Stored" : "Pending"}</Badge></li>
          </ul>
        </div>
      </section>
