      ...
      <div className="source-viewer">
        ...
        <div className="source-code" role="region" aria-label={`Numbered source for ${selected?.name}`}>
          {selected?.normalized_text.split("\n").map((line, index) => {
            ...
            return <div key={index} className={`source-line ${highlighted.has(index + 1) || match ? "highlight" : ""}`}><span className="line-no">{index + 1}</span><span className="line-text">{line || " "}</span></div>;
          })}
        </div>
      </div>
      <aside className="source-side">
        <h3>Linked rules · {linkedRules.length}</h3>
        {linkedRules.map((rule) => <div className="linked-card" key={rule.id}><Badge tone={rule.status === "approved" ? "teal" : "amber"}>{rule.status.replaceAll("_", " ")}</Badge><strong>{rule.title}</strong><p>Lines {rule.source_refs[0]?.line_start}–{rule.source_refs[0]?.line_end} · {rule.enforcement}</p></div>)}
