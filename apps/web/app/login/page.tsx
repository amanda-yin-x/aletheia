...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  ...
  if (!config) {
    ...
    return <main className="auth-page"><section className="auth-card"><div className="auth-brand"><BrandLockup size="large" /></div><h1>Authentication is unavailable.</h1><p className="auth-lede">The deployment is missing its Supabase runtime configuration. Ask the site operator to configure it before signing in.</p></section></main>;
  }
